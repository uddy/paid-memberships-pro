<?php
	...
	{		
		...
		if($pmpro_requirebilling && $gateway != "paypalexpress" && $gateway != "paypalstandard" && $gateway != "twocheckout" && $gateway != "gtpay")
		{									
			...
			{								
				...
				{				
					...
					if($pmpro_requirebilling || ($gateway == "check" && !pmpro_isLevelFree($pmpro_level)))
					{
						...
						elseif($gateway == "gtpay")
						{
							$morder->payment_type = "GTPay WebPay";
							$morder->cardtype = "";
							$morder->ProfileStartDate = date("Y-m-d", strtotime("+ " . $morder->BillingFrequency . " " . $morder->BillingPeriod)) . "T0:0:0";
							$morder->ProfileStartDate = apply_filters("pmpro_profile_start_date", $morder->ProfileStartDate, $morder);							
							$pmpro_processed = $morder->process($morder);
							
							//if($pmpro_processed && !pmpro_isLevelFree($pmpro_level))
							//{
							//	$morder->Gateway->sendToWebPay($morder);
							//}
						}
						else
						{
