<?php
	...
	{                   
		...
		//interswitch stuff here
		pmpro_setOption("Interswitchcheckout_product_id");
		pmpro_setOption("Interswitchcheckout_pay_item_id");
		pmpro_setOption("Interswitchcheckout_mac_key");
		pmpro_setOption("Interswitchcheckout_return_url");
	...
	//interswitch stuff here
	$Interswitchcheckout_product_id = pmpro_getOption("Interswitchcheckout_product_id");
	$Interswitchcheckout_pay_item_id = pmpro_getOption("Interswitchcheckout_pay_item_id");
	$Interswitchcheckout_mac_key = pmpro_getOption("Interswitchcheckout_mac_key");
	$Interswitchcheckout_return_url = pmpro_getOption("Interswitchcheckout_return_url");
...
?>
	...
	<form action="" method="post" enctype="multipart/form-data">         
		...
		<tbody>                		   
			<tr>
				...
				<td>
					<select id="gateway" name="gateway" onchange="pmpro_changeGateway(jQuery(this).val());">
						...
						<option value="interswitch" <?php selected( $gateway, "interswitch" ); ?>>Interswitch</option>
					</select>                        
				</td>
			</tr>
		   ...
		   <tr class="gateway gateway_interswitch" <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
		   <th scope="row" valign="top">	
					<label for="interswitch_partner">Interswitch info here:Note will work only with Naira currency</label>
				</th>
				<td>
				</td>
		   </tr>
		   ...
		   <tr class="gateway gateway_paypal gateway_paypalexpress gateway_paypalstandard" <?php if($gateway != "paypal" && $gateway != "paypalexpress" && $gateway != "paypalstandard") { ?>style="display: none;"<?php } ?>>
			...
			<tr class="gateway gateway_interswitch" <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="Interswitchcheckout_product_id"><?php _e('Interswitch Product ID', 'pmpro');?>:</label>
				</th>
				<td>
					<input type="text" id="Interswitchcheckout_product_id" name="Interswitchcheckout_product_id" size="60" value="<?php echo esc_attr($Interswitchcheckout_product_id)?>" />
				</td>
			</tr>
			<tr class="gateway gateway_interswitch" <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="Interswitchcheckout_pay_item_id"><?php _e('Interswitch Pay Item ID', 'pmpro');?>:</label>
				</th>
				<td>
					<input type="text" id="Interswitchcheckout_pay_item_id" name="Interswitchcheckout_pay_item_id" size="60" value="<?php echo esc_attr($Interswitchcheckout_pay_item_id)?>" />
				</td>
			</tr>
			<tr class="gateway gateway_interswitch" <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="Interswitchcheckout_mac_key"><?php _e('Interswitch Mac Key', 'pmpro');?>:</label>
				</th>
				<td>
					<input type="text" name="Interswitchcheckout_mac_key" size="60" value="<?php echo $Interswitchcheckout_mac_key?>" />
				</td>
			</tr>
			<tr class="gateway gateway_interswitch" <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label for="Interswitchcheckout_return_url"><?php _e('Site redirect URL', 'pmpro');?>:</label>
				</th>
				<td>
					<input type="text" name="Interswitchcheckout_return_url" size="60" value="<?php echo $Interswitchcheckout_return_url?>" />
				</td>
			</tr>
			...
			<tr class="gateway gateway_ gateway_stripe gateway_authorizenet gateway_paypal gateway_payflowpro gateway_braintree gateway_twocheckout  gateway_interswitch gateway_cybersource" <?php if(!empty($gateway) && $gateway != "authorizenet" && $gateway != "paypal" && $gateway != "stripe" && $gateway != "payflowpro" && $gateway != "braintree" && $gateway != "twocheckout" && $gateway != "interswitch" && $gateway != "cybersource") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
			...
			<tr class="gateway gateway_ gateway_stripe gateway_authorizenet gateway_paypal gateway_paypalexpress gateway_check gateway_paypalstandard gateway_payflowpro gateway_braintree gateway_twocheckout gateway_interswitch gateway_cybersource" <?php if(!empty($gateway) && $gateway != "stripe" && $gateway != "authorizenet" && $gateway != "paypal" && $gateway != "paypalexpress" && $gateway != "check" && $gateway != "paypalstandard" && $gateway != "payflowpro" && $gateway != "braintree" && $gateway != "interswitch" && $gateway != "twocheckout" && $gateway != "cybersource") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
			...
			<tr class="gateway gateway_ gateway_stripe gateway_interswitch gateway_paypalexpress gateway_check gateway_paypalstandard gateway_braintree gateway_twocheckout gateway_cybersource" <?php if(!empty($gateway) && $gateway != "stripe" && $gateway != "paypalexpress" && $gateway != "check" && $gateway != "paypalstandard" && $gateway != "braintree" && $gateway != "twocheckout" && $gateway != "cybersource" && $gateway !="interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
		   ...
		   <tr class="gateway gateway_paypal gateway_twocheckout " <?php if($gateway != "twocheckout") { ?>style="display: none;"<?php } ?>>
			...
			<tr class="gateway gateway_paypal gateway_interswitch " <?php if($gateway != "interswitch") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
					<label><?php _e('InterSwitch INS URL', 'pmpro');?>:</label>
				</th>
				<td>
					<p><?php _e('To fully integrate with TwoCheckout, be sure to set your TwoCheckout INS URL ', 'pmpro');?> <pre><?php echo admin_url("admin-ajax.php") . "?action=interswitchcheckout-ins";?></pre>.</p>
				</td>
			</tr>
			<tr class="gateway gateway_authorizenet" <?php if($gateway != "authorizenet") { ?>style="display: none;"<?php } ?>>
				<th scope="row" valign="top">
