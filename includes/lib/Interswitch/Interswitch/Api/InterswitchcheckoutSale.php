...
class Interswitchcheckout_Sale extends Interswitchcheckout
{
   ...
   /*  public static function retrieve($params=array(), $format='json')
    ...
    public static function refund($params=array(), $format='json') {
        ...
        if(array_key_exists("lineitem_id",$params)) {
            ...
            $result = Interswitchcheckout_Message::message('Error', 'You must pass a sale_id, invoice_id or lineitem_id to use this method.');
        }
        return Interswitchcheckout_Util::return_resp($result, $format);
    }
    ...
    public static function stop($params=array(), $format='json') {
        ...
        if(array_key_exists("lineitem_id",$params)) {
            ...
            $result = Interswitchcheckout_Sale::retrieve($params, 'array');
            $lineitemData = Interswitchcheckout_Util::get_recurring_lineitems($result);
            if (isset($lineitemData[0])) {
                ...
                $result = Interswitchcheckout_Message::message('OK', $stoppedLineitems);
            } else {
                throw new Interswitchcheckout_Error("No recurring lineitems to stop.");
            }
        } else {
            throw new Interswitchcheckout_Error('You must pass a sale_id or lineitem_id to use this method.');
        }
        return Interswitchcheckout_Util::return_resp($result, $format);
    }
    ...
    public static function active($params=array(), $format='json') {
        if(array_key_exists("sale_id",$params)) {
            $result = Interswitchcheckout_Sale::retrieve($params);
            $array = Interswitchcheckout_Util::return_resp($result, 'array');
            $lineitemData = Interswitchcheckout_Util::get_recurring_lineitems($array);
            if (isset($lineitemData[0])) {
                $result = Interswitchcheckout_Message::message('OK', $lineitemData);
                if ($format == 'array') {
                    return Interswitchcheckout_Util::return_resp($result, $format);
                } else {
                    return Interswitchcheckout_Util::return_resp($result, 'force_json');
                }
            } else {
                throw new Interswitchcheckout_Error("No active recurring lineitems.");
            }
        } else {
            throw new Interswitchcheckout_Error("You must pass a sale_id to use this method.");
        }
    }
